...
async function main() {
    ...
    const myToken = await MyToken.deploy("GRULL", "GRL", 1000000); // 1 million tokens
    ...
    const tokenAwarder = await TokenAwarder.deploy(myToken.address, 100); // Pass the token address
    ...
    const tokenData = {
      ...
      fs.writeFileSync('./src/contracts/Address.json', JSON.stringify(tokenData));
